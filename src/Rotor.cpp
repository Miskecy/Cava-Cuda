...
Rotor::Rotor(const std::string &inputFile, int compMode, int searchMode, int coinType, bool useGpu,
			 ...
			 const std::string &rangeStart, const std::string &rangeEnd, bool &should_exit, bool stopOnFound, bool quietMode, int color)
{
...
void Rotor::output(std::string addr, std::string pAddr, std::string pAddrHex, std::string pubKey)
{
	...
	// Always write to file; only print to screen when not in quiet mode
...
#define FILTER_SIZE 8
	...
	while (isAlive(params))
	{
			...
			if (isAlive(params))
			{
				...
				if (avgGpuKeyRate > 1000000000)
				{
					...
					if (quietMode)
					{
						double bitsD = 0.0;
						if (count > 0)
							bitsD = log2((double)count);
						printf("\r[%.2f Gkeys/s][%s (2^%.2f | %.6f %%)][%s RUN][Found %d]   ",
				...
				else
				{
					...
					if (quietMode)
					{
						...
						if (count > 0)
							bitsD = log2((double)count);
						printf("\r[%.2f Mkeys/s][%s (2^%.2f | %.6f %%)][%s RUN][Found %d]   ",
		...
		t0 = t1;
		if (should_exit || (stopOnFound && (nbFoundKey >= targetCounter)) || completedPerc > 100.5)
			endOfSearch = true;
	}
...
void Rotor::saveProgress()
{
	...
	if (progressFile.tellp() == 0)
	{ // Verifica se o tamanho do arquivo é 0
